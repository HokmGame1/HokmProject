...
public class Room implements Serializable {
    ...
    private final List<com.yourpackage.Player> players;
    ...
    private com.yourpackage.Deck deck;
    private com.yourpackage.Player master;
    private String hokmSuit;
    private int currentRound;
    private int totalRounds;
    private int currentPlayerIndex;
    private Map<com.yourpackage.Player, com.yourpackage.Card> currentTurnCards;
    ...
    public Room(String creator, int maxPlayers, int totalRounds) {
        ...
        this.deck = new com.yourpackage.Deck();
        this.currentRound = 0;
    ...
    public List<com.yourpackage.Player> getPlayers() {
        return players;
    }

    public com.yourpackage.Player getPlayerByName(String username) {
    ...
    public synchronized boolean removePlayer(String username) {
        com.yourpackage.Player player = getPlayerByName(username);
        if (player != null) {
    ...
    public synchronized void addPlayer(com.yourpackage.Player player) {
    ...
    private void addPlayerToTeam(com.yourpackage.Player player) {
    ...
    private void dealInitialCards() {
        players.forEach(player -> {
            List<com.yourpackage.Card> initialCards = deck.drawCards(5);
    ...
    private void dealRemainingCards() {
        players.forEach(player -> {
            List<com.yourpackage.Card> remainingCards = deck.drawCards(13 - player.getHand().size());
            player.getHand().addAll(remainingCards);
            sendCardsToPlayer(player);
        });
    }

    private void sendCardsToPlayer(com.yourpackage.Player player) {
    ...
    private void nextTurn() {
        ...
        com.yourpackage.Player currentPlayer = players.get(currentPlayerIndex);
        broadcastMessage("PLAYER_TURN:" + currentPlayer.getName());
    }

    public synchronized void playCard(com.yourpackage.Player player, com.yourpackage.Card card) {
    ...
    private void determineTurnWinner() {
        com.yourpackage.Player winner = null;
        com.yourpackage.Card winningCard = null;

        for (Map.Entry<com.yourpackage.Player, com.yourpackage.Card> entry : currentTurnCards.entrySet()) {
            com.yourpackage.Player player = entry.getKey();
            com.yourpackage.Card card = entry.getValue();
    ...
    private boolean isCardHigher(com.yourpackage.Card card1, com.yourpackage.Card card2) {
    ...
    private void sendMessageToPlayer(com.yourpackage.Player player, String message) {
        try {
    ...
    private void notifyPlayerKicked(com.yourpackage.Player player) {
        sendMessageToPlayer(player, "KICKED");
        player.closeConnections();
    }

    private void closeAllConnections() {
        for (com.yourpackage.Player player : players) {
            try {
